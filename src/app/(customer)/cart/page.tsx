...
const ITEM_DETAILS = PRODUCTS.reduce((acc, product) => {
  acc[product.name] = {
    ...
    categoryEn: product.tagsEn[0] || "Keto",
...
export default function CartPage() {
  ...
  const handleWhatsAppCheckout = async () => {
    if (storeStatus === "closed") {
      alert(t.cartAlertClosed);
      return;
    }
    if (!clientName.trim()) {
      alert(t.cartAlertEnterName);
      return;
    }
    if (!address.trim()) {
      alert(t.cartAlertEnterAddress);
      return;
    }
    if (cartItems.length === 0) {
      alert(t.cartAlertEmpty);
      return;
    }

    const orderId = "ord-" + Math.floor(100000 + Math.random() * 900000);
    const detailsStr = cartItems.map((item) => `${item.count}x ${item.title}`).join(", ");
    const formattedTotal = formatCOP(total);
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-7 lg:col-span-8 flex flex-col gap-6">
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-display font-bold text-3xl text-on-surface">{t.cartTitle}</h2>
              <span className="font-sans font-bold text-sm text-emerald-800 bg-emerald-100/80 px-3.5 py-1 rounded-full border border-emerald-500/20">
                {cartRaw.length} {cartRaw.length === 1 ? t.cartItemSingle : t.cartItemPlural}
              </span>
            </div>

            {cartItems.length === 0 ? (
              <div className="bg-surface-container-lowest rounded-2xl p-12 text-center border border-emerald-500/10 shadow-[0px_4px_20px_rgba(0,0,0,0.03)] flex flex-col items-center gap-4">
                <span className="material-symbols-outlined text-[64px] text-emerald-600/40">shopping_cart</span>
                <p className="font-sans text-on-surface-variant text-base">{t.cartEmpty}</p>
                <Link href="/" className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-sm px-6 py-3.5 rounded-full active:scale-95 transition-all shadow-md shadow-emerald-600/20">
                  {t.cartExploreBtn}
                </Link>
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                {cartItems.map((item) => {
                  ...
                  return (
                    <div
                      key={item.title}
                      className="bg-surface-container-lowest rounded-2xl p-6 flex flex-col sm:flex-row gap-6 shadow-[0px_4px_20px_rgba(0,0,0,0.03)] hover:-translate-y-0.5 transition-transform duration-300 border border-emerald-500/10"
                    >
                      <div className="w-full sm:w-28 h-28 rounded-xl overflow-hidden shrink-0 relative bg-surface-container-low">
                        <img className="w-full h-full object-cover" src={details.imageUrl} alt={language === "en" ? details.nameEn : item.title} />
                      </div>
                      <div className="flex flex-col justify-between flex-grow">
                        <div>
                          <div className="flex justify-between items-start mb-2">
                            <h3 className="font-display font-bold text-lg text-on-surface">{language === "en" ? details.nameEn : item.title}</h3>
                          ...
                          <span className="font-sans font-bold text-xs bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-md border border-emerald-500/15">
                            {language === "en" ? details.categoryEn : details.category}
                          </span>
                        </div>
                        <div className="flex justify-between items-center mt-4">
                          <button
                            onClick={() => removeFromCart(item.title)}
                            className="text-on-surface-variant hover:text-error text-xs font-semibold flex items-center gap-1 transition-colors"
                          >
                            <span className="material-symbols-outlined text-base">delete</span>
                            <span>{language === "en" ? "Remove" : "Eliminar"}</span>
                          </button>
                          
                          {/* Quantity Controls */}
                          <div className="flex items-center gap-3 bg-surface-container-low px-2 py-1 rounded-lg border border-outline-variant/10">
                            <button
                            ...
                            <span className="font-sans font-bold text-sm w-4 text-center">{item.count}</span>
                            <button
          ...
          <div className="md:col-span-5 lg:col-span-4 mt-4 md:mt-0">
            <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-[0px_10px_40px_rgba(5,150,105,0.06)] border border-emerald-500/15 sticky top-28 flex flex-col gap-6">
              <h2 className="font-display font-bold text-xl text-on-surface border-b border-emerald-500/10 pb-3">
                {t.cartSummaryTitle}
              </h2>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between font-sans text-sm text-on-surface-variant">
                  ...
                  <span className="text-emerald-700 font-bold">{formatCOP(deliveryFee)}</span>
                </div>
                <div className="h-px bg-emerald-500/10 w-full my-2"></div>
                <div className="flex justify-between font-display font-black text-xl text-on-surface">
                  <span>{t.cartTotal}</span>
                  <span className="bg-gradient-to-r from-emerald-700 to-teal-700 bg-clip-text text-transparent">{formatCOP(total)}</span>
                </div>
              </div>
              ...
              {cartItems.length > 0 && (
                <div className="flex flex-col gap-4 mt-2">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="clientNameInput" className="font-sans font-bold text-xs text-on-surface-variant uppercase tracking-wider">
                      {t.cartNameLabel}
                    </label>
                    <input
                      id="clientNameInput"
                      type="text"
                      className="w-full bg-surface-container-low rounded-xl px-4 py-3 font-sans text-sm text-on-surface border border-outline-variant/10 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all outline-none"
                  ...
                  {/* Address Input */}
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="addressInput" className="font-sans font-bold text-xs text-on-surface-variant uppercase tracking-wider">
                      {t.cartAddressLabel}
                    </label>
                    <input
                      id="addressInput"
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-sans font-bold text-sm py-4 rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/25 mt-2"
                  >
